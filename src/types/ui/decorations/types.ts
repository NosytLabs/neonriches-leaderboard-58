...
export type MedievalSize = 'sm' | 'md' | 'lg' | 'xl' | '2xl' | 'xs';
...
export const MEDIEVAL_ICON_SIZES: Record<MedievalSize, { container: string; icon: MedievalSize; border: string; }> = {
  'xs': {
    container: 'w-4 h-4',
...
export const MEDIEVAL_ICON_BG_COLORS: Record<MedievalIconColor, string> = {
  ...
  'platinum': 'from-indigo-300/20 to-indigo-400/10 border-indigo-300/30'
};

// Royal Divider specific types
...
export type MedievalDecorationSize = MedievalSize;
...
export const getColorClass = (color: MedievalDecorationColor = 'gold', prefix: string = 'text'): string => {
