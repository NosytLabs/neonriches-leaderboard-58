...
const faqSections = [
  ...
  },
  {
    ...
    icon: <Shield className="mr-2 h-5 w-5" aria-hidden="true" />,
    questions: [
      {
        id: 'privacy-question',
        question: 'How is my personal information protected?',
        answer: 'We take privacy seriously, even in our satire. We collect only the information necessary to run the platform, and all payments are processed through secure third-party providers. We never store your full payment details. For complete information, please see our <Link to="/privacy" className="text-royal-gold hover:underline">Privacy Policy</Link>.'
      },
      {
        id: 'is-this-legit',
        question: 'Is this site legitimate/legal?',
        answer: 'Yes! SpendThrone is a legitimate satirical art project and social experiment. We operate transparently, with clear terms of service and privacy policies. Our satirical nature is explicitly stated throughout the site. We comply with all relevant laws regarding online commerce, data privacy, and content moderation. Remember though, all payments are non-refundable as clearly stated in our <Link to="/terms" className="text-royal-gold hover:underline">Terms of Service</Link>.'
      }
    ]
  },
  {
    id: 'philosophy',
    title: 'Philosophy & Purpose',
    icon: <Gem className="mr-2 h-5 w-5" aria-hidden="true" />,
    questions: [
      {
        id: 'what-is-point',
        question: 'What\'s the actual point of all this?',
        answer: 'The point is to make visible what is often invisible: the direct relationship between money and status in modern society. By creating a system where status is explicitly and solely tied to spending, we highlight how many real-world status systems function similarly but less honestly. It\'s satire that asks: If you\'d pay for a rank here, what other meaningless status symbols are you paying for elsewhere? And why?'
      },
      {
        id: 'charity-question',
        question: 'Does any of this money go to charity?',
        answer: 'A portion of the prize pool is satirically allocated to "charity" as part of our commentary on how the wealthy often use charitable giving as another status symbol. In reality, we do donate a percentage of profits to organizations addressing wealth inequality and providing essential services to those in need. We just don\'t think charity should be a status-enhancing performance, which is why we don\'t emphasize it in our royal branding.'
      },
      {
        id: 'creator-question',
        question: 'Who created SpendThrone and why?',
...
const FAQ = () => {
  ...
  const faqSchemaData = generateFAQSchema(allQuestions);
  const faqJsonLd = generateJsonLd(faqSchemaData);
  
  const seoKeywords = [
    'SpendThrone FAQ', 
    'pay to win FAQ', 
    'digital nobility questions', 
    'social experiment rules',
    'satirical social platform',
  ...
  return (
    <Layout
      ...
      description="Get answers to common questions about SpendThrone, the satirical pay-to-win social experiment where your rank is determined by spending."
      fullHeight
    >
      <SEO
        title="Frequently Asked Questions | SpendThrone"
        description="Find answers to common questions about SpendThrone, our satirical pay-to-win social experiment where your rank is determined by how much you spend."
      ...
      <div className="container mx-auto max-w-4xl relative">
        <div className="text-center mb-10">
          <div className="inline-flex mb-4">
            <MedievalIcon name="scroll" size="lg" color="gold" className="animate-crown-glow" aria-hidden="true" />
          </div>
        ...
        <div className="space-y-8">
          {faqSections.map((section) => (
            <Card key={section.id} className="glass-morphism border-white/10 p-6">
              <h2 className="text-2xl font-royal text-royal-gold mb-4 flex items-center" id={`section-${section.id}`}>
                {section.icon}
                {section.title}
              </h2>
