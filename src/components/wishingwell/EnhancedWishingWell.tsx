...
const EnhancedWishingWell: React.FC = () => {
  ...
  const handleWish = async () => {
    ...
    // Calculate the total amount spent (from any of these properties)
    const totalSpent = user.totalSpent || user.amountSpent || user.spentAmount || 0;
    ...
    try {
      const userForTransaction = ensureUser(user);
        ...
        <div className="space-y-4">
          <div>
            <div className="flex justify-between mb-2">
              ...
              <span className="text-sm font-bold">{formatDollarAmount(wishAmount)}</span>
            </div>
            <Slider 
              value={[wishAmount]} 
              min={0.25} 
