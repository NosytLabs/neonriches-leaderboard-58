...
import { Sparkles, Gem, Coins, Gift, ArrowDownCircle, Info } from 'lucide-react';
...
const EnhancedWishingWell = () => {
  ...
  const predefinedAmounts = [0.25, 0.5, 1, 2, 5, 10, 25];
  ...
  const handleWish = async () => {
    ...
    if ((user.walletBalance || 0) < wishAmount) {
      toast({
    ...
    try {
      ...
      await updateUserProfile({
        ...user,
        walletBalance: (user.walletBalance || 0) - wishAmount,
        totalSpent: (user.totalSpent || 0) + wishAmount,
        amountSpent: (user.amountSpent || 0) + wishAmount,
        spentAmount: (user.spentAmount || 0) + wishAmount,
      });
      ...
      if (noReward) {
        ...
        const { cosmeticItem, rarity } = await awardRandomCosmetic(wishAmount, preferredCategory);
  ...
  const awardRandomCosmetic = async (amount: number, category?: string) => {
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                <span className="text-sm text-white/70 mr-2">Preferred Category (Optional):</span>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="text-xs">Choose a category to increase your chance of getting items from that category.</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
          ...
          <div className="p-3 bg-black/20 rounded-lg text-sm">
            ...
            <div className="grid grid-cols-6 gap-x-1 gap-y-2 my-3">
              ...
              <div className="text-center">
                <div className="text-xs font-semibold text-red-400">Royal</div>
                <div className="text-xs text-white/50">{wishAmount >= 25 ? 5 : 0}%</div>
              </div>
            </div>
